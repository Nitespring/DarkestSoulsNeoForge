...
public class FrayedBlade extends Weapon {
    ...
    public FrayedBlade(Tier tier, float attack, float speed, float reach, float knockback, int poise, int posture, int blood, int poison, int frost, int rot, int death, int fire, int holy,int serrated, int durability, int enchantability, float movementSpeed, int maxTargets, Properties properties) {
    ...
    public void doLeftClickAction(Player playerIn, ItemStack stackIn) {
        if(CommonConfig.do_special_weapon_attacks_left_click.get()) {
            if (!playerIn.isUsingItem()) {
                ...
                entity.setDamage(
                        Math.round(this.getSweepAttackDamage(playerIn, stackIn) *0.2f),
                        Math.round(this.getSweepPoiseDamage(playerIn, stackIn) * 0.1f),
                        Math.round(this.getSweepPostureDamage(playerIn, stackIn) *0.1f),
