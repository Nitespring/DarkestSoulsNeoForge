...
public class BeastPatientSpawnRandomizer extends EntitySpawnRandomizer {


    public BeastPatientSpawnRandomizer(EntityType<? extends PathfinderMob> p_21683_, Level p_21684_) {
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty, MobSpawnType spawnType, @Nullable SpawnGroupData spawnGroupData) {
        ...
        switch(r.nextInt(15)) {
            case 1,2,3,4,5:
                ...
                break;
            case 6,7:
                mob = new AshenBloodBeastPatient(EntityInit.ASHEN_BLOOD_BEAST_PATIENT.get(), level.getLevel());
                mob.finalizeSpawn(level, difficulty, spawnType, spawnGroupData);
                finalizeMobSpawn(mob);
                if(CommonConfig.spawn_ashen_blood_beast_patient.get()){level.getLevel().addFreshEntity(mob);}
                this.discard();
                break;
            default :
