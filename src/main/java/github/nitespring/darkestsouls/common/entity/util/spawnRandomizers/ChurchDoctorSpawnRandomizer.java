...
public class ChurchDoctorSpawnRandomizer extends EntitySpawnRandomizer {


    public ChurchDoctorSpawnRandomizer(EntityType<? extends PathfinderMob> p_21683_, Level p_21684_) {
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty, MobSpawnType spawnType, @Nullable SpawnGroupData spawnGroupData) {
        ...
        switch(r.nextInt(15)) {
            case 1,2,3,4:
                mob = new ChurchDoctorLantern(EntityInit.CHURCH_DOCTOR_LANTERN.get(), level.getLevel());
                mob.finalizeSpawn(level, difficulty, spawnType, spawnGroupData);
                finalizeMobSpawn(mob);
                if(CommonConfig.spawn_church_doctor.get()){level.getLevel().addFreshEntity(mob);}
                this.discard();
                break;
            case 5,6,7:
                mob = new ChurchDoctorLantern(EntityInit.CHURCH_DOCTOR_PISTOL.get(), level.getLevel());
                mob.finalizeSpawn(level, difficulty, spawnType, spawnGroupData);
                finalizeMobSpawn(mob);
                if(CommonConfig.spawn_church_doctor_pistol.get()){level.getLevel().addFreshEntity(mob);}
                this.discard();
                break;
            case 8,9:
            ...
            case 10:
                ...
                break;
            default :
                mob = new ChurchDoctorStick(EntityInit.CHURCH_DOCTOR.get(), level.getLevel());
                mob.finalizeSpawn(level, difficulty, spawnType, spawnGroupData);
                finalizeMobSpawn(mob);
                if(CommonConfig.spawn_church_doctor.get()){level.getLevel().addFreshEntity(mob);}
                this.discard();
